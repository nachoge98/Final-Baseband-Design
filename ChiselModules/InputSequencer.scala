package chipyard.baseband//inSequencer
...
class ISInputBundle extends Bundle {//correction block inputs
  ...
  val data = Flipped(Decoupled(UInt(24.W)))
...
class InputSequencer extends Module {
  ...
  val in_data = Wire(UInt(24.W))//RegNext(io.in.data.bits, 0.U)//Wire(UInt(8.W))
  ...
  val reminder = RegInit(2.U(2.W))//4.U(3.W
  ...
  val idle :: s1 :: s2 :: s3 :: waitData :: Nil = Enum(5)//:: s4 
  ...
  when(state === idle){//STATE: IDLE
    ...
    when(in_data_valid){
      ...
      data_bits1 := in_data(23,16)//in_data(31,24)
      data_bits2 := in_data(15,8)//in_data(23,16)
      data_bits3 := in_data(7,0)//in_data(15,8)
  ...
  */
  ...
  out_data := Mux(state===s1, data_bits1, Mux(state===s2, data_bits2, data_bits3))
  //Mux(state===s1, data_bits1, Mux(state===s2, data_bits2, Mux(state===s3, data_bits3, data_bits4)))
  
  when(in_reminder_valid){
    reminder := in_reminder
  }
  
  out_data_valid := Mux(out_data_ready && (state === s1 || state === s2 || state === s3) && !soft_reset, true.B, false.B)
  //Mux(out_data_ready && (state === s1 || state === s2 || state === s3 || state === s4) && !soft_reset, true.B, false.B)
