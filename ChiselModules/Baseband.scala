...
class BasebandInputBundle extends Bundle { //cambiado el input bundle para anadir mi enable, y la AA.
  ...
  val data = Output(UInt(1.W))//Flipped(Decoupled(UInt(1.W))) //change to adapt it to a fifo.
...
class BasebandOutputBundle extends Bundle {
  ...
  val data = Output(UInt(1.W))//Decoupled(UInt(1.W)) //change to adapt it to a fifo.
...
class BasebandIO extends Bundle { //I have adapted the in/out to the antenna to fit a FIFO for simulation.
...
class Baseband extends Module {
  ...
  bp_in_rxData := io.in.bits.data//io.in.data.bits
  val bp_in_rxData_valid = Wire(Bool())
  bp_in_rxData_valid := io.in.valid//io.in.data.valid//false.B
  ...
  val inFIFO_data = Wire(UInt(32.W))
  inFIFO_data := io.inFIFO.bits.data
  ...
  val idle_listAA :: length_listAAs :: receiving_AAs ::  Nil = Enum(3)
  ...
  val reg_packet = RegInit(VecInit(Seq.fill(63)(0.U(32.W)))) //register data packet, usar vector en grupos de 32 en 32 //251 bytes max /4=62.7 -> 63
  ...
  val reg_num32_packet = RegInit(0.U(6.W)) //indicates the number of 32 bit sections that contain data. // 251/4 = 62.7 -> 63.
  val reg_payLength_packet = RegInit(0.U(8.W)) //necessary max 251 bytes //2^8 = 256
  val reg_reminder_packet = RegInit(0.U(3.W)) //values can be 0,1,2,3
  
  val write_idx_packet = RegInit(0.U(6.W))
  val read_idx_packet = RegInit(0.U(6.W))
  ...
  val idle_txMode :: rec_length_txMode :: rec_AA_txMode :: saved_AA_txMode :: delayState_txMode :: transmitting_txMode :: transmitting_savedMode ::  Nil = Enum(7)
  val state_txMode = RegInit(idle_txMode)
  
  val reg_num32_normalTx = RegInit(0.U(6.W)) //indicates the number of 32 bit sections that contain data. // 251/4 = 62.7 -> 63.
  ...
  val reg_rec32_normalTx = RegInit(0.U(6.W)) //indicates the number of 32 bit sections of data received.
  
  val txMode_access_address = RegInit(0.U(32.W))
  ...
  val control_data = RegInit(0.U(8.W))//Wire(UInt(8.W))//RegInit(0.U(8.W))//TIENEN QUE SER WIRES
  control_data := io.con.bits.data
  ...
  //REGISTERS STATUS:
  val reg_channel = RegInit(0.U(8.W)) //probably doesnt need 8 bits, creo que eran 5 //COMPROBAR
  ...
  when(control_valid && control_data(4) && control_data(2)){ //soft reset input
  ...
  switch (state) {
    is (idle) {
      ...
      when(control_valid  && !control_data(7) && control_data(5) && control_data(0)){
      
        state := clearAA
        
      }.elsewhen(control_valid && !control_data(7) && control_data(5) && control_data(1) ){
      
        state := rxOne
        state_rxMode := listening_rxMode
        
      }.elsewhen(control_valid && !control_data(7) && control_data(5) && control_data(2) ){
      
        state := rxContinuous
        state_rxMode := listening_rxMode
        
      }.elsewhen(control_valid && !control_data(7) && control_data(6) && control_data(0) ){
      
        state := sendStatus
        
      }.elsewhen(control_valid && !control_data(7) && control_data(6) && control_data(1) ){
      ...
      }.elsewhen(control_valid && !control_data(7) && control_data(6) && control_data(2) ){
      ...
      }.elsewhen(control_valid && control_data(7) && control_data(5) && control_data(0) ){
        ...
        state := savePacket
        state_regPacket := length_regPacket
        
      }.elsewhen(control_valid && control_data(7) && control_data(5) && control_data(3) ){
        ...
        when(reg_num32_packet =/= 0.U){
          state := txSavedAA
          state_txMode := rec_AA_txMode//rec_length_txMode
        }.otherwise{
          state := idle
          state_txMode := idle_txMode
        }
        ...
        state := tx
        state_txMode := rec_length_txMode
        
      }.elsewhen(control_valid && control_data(7) && control_data(6) && control_data(2) ){
      
        state := saveListAA
        state_listAA := length_listAAs
        
      }.elsewhen(control_valid && control_data(4) && control_data(0) ){
      
        state := actEC
        
      }.elsewhen(control_valid && control_data(4) && control_data(1) ){
  ...
  switch (state_listAA) {
    ...
    is (receiving_AAs) {
      ...
      when(inFIFO_valid && (num_AA_list <= 10.U) && !soft_reset){//has to be minor than the max number of AAs I can store right now, I can change it.
        list_AAs(write_idx_AA_list) := inFIFO_data
  ...
  listAA_inFIFO_ready := MuxCase(false.B, Array((state_listAA === length_listAAs) -> true.B, (state_listAA === receiving_AAs) -> true.B))
  ...
  switch (state_regPacket) {
    ...
    is (length_regPacket) {
      ...
      when(inFIFO_valid){
        reg_num32_packet := inFIFO_data(5,0) //6bits
        reg_payLength_packet := inFIFO_data(13,6) //8bits
        reg_reminder_packet := (inFIFO_data(13,6).asUInt + 2.U) % 4.U //3bits
        state_regPacket := receiving_regPacket 
      }
      write_idx_packet := 0.U //6bits
      read_idx_packet := 0.U
  ...
  switch (state_rxMode) {
    ...
    is (listening_rxMode) {
      when(control_valid && !control_data(7) && control_data(5) && control_data(3)){
        state := idle
  ...
  //State Machine Tx Modes
    switch (state_txMode) {
      ...
      is (rec_AA_txMode) {
        ...
        when(inFIFO_valid){
          txMode_access_address := inFIFO_data
  ...
  tx_inFIFO_ready := MuxCase(false.B,
                           Array((state_txMode === rec_length_txMode) -> true.B,
                                 (state_txMode === rec_AA_txMode) -> true.B,
  ...
  //input controller
  errCorr.io.inCon.enableErrCorr := ec_con_enable
  ...
  -comprobar bit width del transmission power, y del resto de basuras del status.
  ...
  */
  ...
  io.inFIFO.ready := MuxCase(false.B,
                           Array((state === saveListAA || state === saveAA) -> listAA_inFIFO_ready,
                                 (state === modifyAA) -> true.B,
                                 (state === setTrPower) -> true.B,
                                 (state === savePacket) -> regPacket_inFIFO_ready,
                                 (state === tx) -> tx_inFIFO_ready,
                                 (state === txSavedAA) -> tx_inFIFO_ready))
  
  io.in.ready := rx_inAntenna_ready//bp_in_rxData_ready //io.in.data.ready
  ...
  io.out.bits.data := bp_out_txData //io.out.data.bits
  io.out.valid := bp_out_txData_valid //io.out.data.valid
  bp_out_txData_ready := io.out.ready //io.out.data.ready
...
trait HasPeripheryBaseband extends BaseSubsystem {
  ...
  pbus.toVariableWidthSlave(Some("inDataFIFOWrite")) { baseband.writeQueueDataFIFO.mem.get }
  pbus.toVariableWidthSlave(Some("inControllerWrite")) { baseband.writeQueueControlFIFO.mem.get }
  pbus.toVariableWidthSlave(Some("inAntennaWrite")) { baseband.writeQueueAntennaFIFO.mem.get }
  
  pbus.toVariableWidthSlave(Some("outDataFIFORead")) { baseband.readQueueDataFIFO.mem.get }
  pbus.toVariableWidthSlave(Some("outAntennaRead")) { baseband.readQueueAntennaFIFO.mem.get }
}
